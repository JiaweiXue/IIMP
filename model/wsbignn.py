...
class WSBiGNN(nn.Module):
    ...
    def run(self, x_u_v, x_poi, x_user, x_adj, bs):
        ...
        for i in range(bs):              
            ...
            #M3: search mobility memory
            recent_w_1 = torch.mm(x_result_u, self.s_g_w)    #(nu,nv)
            recent_w_2 = torch.mm(x_result_u, self.s_ng_w)   #(nu,nv)
            recent_w_3 = torch.mm(x_result_u, self.ns_g_w)   #(nu,nv)
            recent_w_4 = torch.mm(x_result_u, self.ns_ng_w)  #(nu,nv)
            recent_w = torch.cat((recent_w_1.unsqueeze(dim=0), recent_w_2.unsqueeze(dim=0),\
                       recent_w_3.unsqueeze(dim=0), recent_w_4.unsqueeze(dim=0)),dim=0)  #(4,nu,nv)
            con_w = torch.cat((self.const[0].expand((1,nu,nv)), self.const[1].expand((1,nu,nv)), \
                               self.const[2].expand((1,nu,nv)), self.const[3].expand((1,nu,nv))), dim=0)  #(4,nu,nv)  
            recent_w = recent_w + con_w    
            historical_w = self.member   #(4, nu, nv)
            inter_w = (recent_w + historical_w)/2.0
            
            pattern_score = torch.zeros((self.nu, self.nv), device=device).float()
            pattern_score = pattern_score + torch.mul(inter_w[0], torch.mm(self.s_g_u, torch.transpose(self.s_g_v,1,0))) 
            pattern_score = pattern_score + torch.mul(inter_w[1], torch.mm(self.s_ng_u, torch.transpose(self.s_ng_v,1,0)))  
            pattern_score = pattern_score + torch.mul(inter_w[2], torch.mm(self.ns_g_u, torch.transpose(self.ns_g_v,1,0))) 
            pattern_score = pattern_score + torch.mul(inter_w[3], torch.mm(self.ns_ng_u, torch.transpose(self.ns_ng_v,1,0)))  
            ...
            u_v_score = pattern_score + u_v_score
